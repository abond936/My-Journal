...
import GalleryItemEditor from '@/components/admin/studio/cards/GalleryItemEditor';
...
export default function StudioCardFormGallery({
  ...
  const appendToGallery = useCallback(
    (items: HydratedGalleryMediaItem[]) => {
      ...
      setField('galleryMedia', [...gallery, ...reindexed]);
    },
    [gallery, setField]
  );
  ...
  const ingestImageFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        setField('galleryMedia', appendGalleryMediaItems(gallery, [media]));
      } catch (error) {
    ...
    [gallery, registerCreatedMedia, setField]
  );

  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      const file = acceptedFiles[0];
      if (!file || disabled || pasteBusy) return;
      void ingestImageFile(file);
    },
    [disabled, ingestImageFile, pasteBusy]
  );
  ...
  return (
    <div>
      <div className={styles.studioGallerySectionHeader}>
      ...
      <p className={styles.metaMuted}>
        Paste or drop an image here, pick from the library, or drag from the Media pane (Studio).
      </p>
      {pasteError ? (
        <p className={styles.studioGalleryPasteError} role="alert">
          {pasteError}
        </p>
      ) : null}
      <div
        ...
        tabIndex={0}
        className={`${styles.studioGalleryPasteTarget} ${isDragActive ? styles.studioGalleryPasteTargetActive : ''}`}
        aria-label="Gallery: paste, drop, or drag media here"
      >
        <input {...getInputProps()} />
        {gallery.length ? (
          <StudioDropZone
                    ...
                    <div className={styles.studioGalleryCard}>
                      <div className={styles.studioGalleryThumbWrap}>
                        {item.media ? (
                          <JournalImage
                            ...
                            style={{
                              objectPosition:
                                item.objectPosition || item.media.objectPosition || '50% 50%',
                            }}
                          />
                        ) : (
                      ...
                      <div className={styles.studioGalleryActions}>
                        ...
                        <button
                          ...
                          aria-label={
                            item.mediaId === currentCoverMediaId
                              ? 'Current cover image'
                              : 'Set as cover image'
                          }
                          title={
