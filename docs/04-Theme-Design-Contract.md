...
**Document role:** Support document. Owns detailed theme-system semantics, token roles, and responsive layout contract details.
**Does not own:** Product vision (`01`), canonical app behavior/status by surface (`02`), or active milestone sequencing (`03`). If a theme decision changes reader/admin product behavior, mirror that truth back into `02`. If it changes active priority, mirror it into `03`.
...
| **Sidebar** | `sidebar -> main` | `surfaces.chromeSidebar`, `typography.chromeText`, `typography.sidebarLabel`, `typography.sidebarCount`, `typography.chromeMeta`, `controls.chromeActiveTab`, `controls.inlineLink`, `iconography.chrome`, plus width tokens | `semantic/reader/chrome-surface`, `semantic/reader/chrome-border`, explicit sidebar text/label/count/meta roles, contrast-on-fill text, accent | Now first-class in the authoring layer and less coupled to support/discovery typography. Sidebar title/navigation text, section and collection labels, compact counts, and remaining secondary metadata are distinct authorable jobs rather than component-level size overrides. |
