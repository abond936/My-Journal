...
📐 **Review program order** - After current Studio slice verification, prefer: **(1)** engineering safety net — shipped 2026-06-12 (CI gate expansion, Playwright smoke tests, `storage.rules`, `.env.example`), **(2)** reader performance — in progress (see step 2 sequencing below), **(3)** backend hardening already tracked under Backend gates, **(4)** Studio legacy retirement, **(5)** reader mobile text edit on top of the stabilized substrate per `01` **Program structure**.
...
- **Storage byte backup** - Add automated, verified, off-site backup and documented restore for Firebase Storage object bytes (originals and renditions), complementing `backup:database`; without this, Firestore restore can leave media rows pointing at missing blobs until separate recovery or re-import.
