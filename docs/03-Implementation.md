...
📐 **External codebase review handoff (2026-06-12)** - A full-repo review found strong integrity engineering, a coherent reader V2 + theme token foundation, and a credible Studio convergence direction. **Review program steps 1-8 shipped 2026-06-12.** Remaining gaps (deferred reader/Studio/engineering follow-ups in steps 9-11, operational recovery closeout in step 12) are tracked below — not a rewrite mandate.

📐 **Review program order** - **Complete 2026-06-12:** **(1)** engineering safety net, **(2)** reader performance, **(3)** backend hardening, **(4)** Studio legacy retirement, **(5)** reader mobile text edit (5a-5d), **(6)** E2E hardening, **(7)** API/service hardening, **(8)** pre-commercial security/ops. **Active:** post-review **step 12** (operational recovery closeout). **Deferred:** steps **9-11** until profiling, demo need, or feature pressure.

📐 **Post-review program order (2026-06-12)** - Continue one approved slice at a time. **Storage restore proof is explicitly last (step 12)** per author direction. **Next:** step **12** operational recovery closeout (Storage restore drill on a disposable target). **Deferred:** steps **9-11** (author 2026-06-12).
...
- **(12) Operational recovery closeout (last)** - **Planned.** Full disposable-target drill: Firestore + Storage bytes together, Typesense/search parity, verification, and hosted-runtime sanity on recovered data. Firestore restore + viewer recovery already proven; `backup:storage` script shipped (3a). Operator contract in `docs/NPM-SCRIPTS.md` -> **Restore drill** (step **3b** Storage restore is manual until an approved `restore:storage` helper). Suggested slices: **12a** confirm recent `backup:storage --apply` run; **12b** disposable-target Firestore + Storage restore + Typesense rebuild; **12c** verification checklist + canon closeout; **12d** *(optional)* guarded `restore:storage` script.
...
📐 **Post-review step 12 — Operational recovery closeout sequencing (2026-06-12)** - **Next active program step.** Implement in this order on a **disposable Firebase project** (never production first):

- **12a Storage backup readiness** - Confirm a recent `npm run backup:storage -- --apply` run exists under `ONEDRIVE_PATH/Firebase Backups/run-<timestamp>/` with `storage-manifest.json` and populated `storage/` (pair with a matching `firestore.json` from the same or compatible run).
- **12b Disposable-target restore drill** - `restore:database` dry-run then apply; manual Storage byte restore per `NPM-SCRIPTS.md` step **3b**; `sync:typesense:fresh` + `sync:typesense:media:fresh`; reconcile theme/admin seed only if needed.
- **12c Verification + canon closeout** - `npm run build`, `npm run test:integrity -- --runInBand`, admin + viewer sign-in, representative cards/media/tags/search on recovered data; reconcile step **12** shipped in `01` / `02` / `03`.
- **12d Restore helper (optional follow-up)** - Guarded `restore:storage` script if manual upload is too painful to repeat; not required to claim first Storage restore proof.
...
- **Operational recovery gate** - Verify database backup, local secrets backup, restore drill, rollback/incident response, and admin account recovery before commercial release. Current closeout: Firestore restore + viewer-account recovery proven on disposable `my-journal-restore-drill`; **Storage byte backup script shipped 2026-06-12** (`npm run backup:storage`). **Next (post-review step 12):** full disposable-target **Storage restore drill** (Firestore + Storage bytes + Typesense rebuild + verification). Operator path: `docs/NPM-SCRIPTS.md` -> **Restore drill**; automated `restore:storage` remains optional follow-up (**12d**).
