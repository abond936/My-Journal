...
📐 **External codebase review handoff (2026-06-12)** - A full-repo review found strong integrity engineering, a coherent reader V2 + theme token foundation, and a credible Studio convergence direction. **Review program steps 1-5 shipped 2026-06-12.** Remaining Fable gaps (E2E hardening, API/service hardening, pre-commercial security/ops, engineering quality, operational recovery closeout) are tracked as **post-review program steps 6-12** below — not a rewrite mandate.
...
📐 **Post-review program order (2026-06-12)** - Continue one approved slice at a time. **Storage restore proof is explicitly last (step 12)** per author direction. **Next:** step **9** reader follow-ups (defer until profiling or demo need).
...
- **(12) Operational recovery closeout (last)** - **Storage restore drill** on a disposable target (Firestore + Storage bytes together). **Then** Typesense/search parity on recovered environment, broader incident-response proof, and deployment/runtime validation on recovered target. Firestore restore + viewer recovery already proven; `backup:storage` script shipped (3a).
...
- **8d Error monitoring** - Baseline production error visibility (Sentry). **Shipped 2026-06-12:** `@sentry/nextjs` + `sentryRuntime.ts`; production-only when DSN env vars set; errors-only (`tracesSampleRate: 0`); `beforeSend` scrubbing; `global-error.tsx`; optional source maps when `SENTRY_AUTH_TOKEN` set; regression in `sentryRuntime.test.ts`.
...
- **Error monitoring** - Baseline production error visibility (Sentry). **Shipped 2026-06-12 (slice 8d):** opt-in `@sentry/nextjs` wiring; production-only when DSN env vars set; errors-only baseline with secret scrubbing.
...
- **Operational recovery gate** - Verify database backup, local secrets backup, restore drill, rollback/incident response, and admin account recovery before commercial release. Current closeout: the backup serializer now reserves `id` for the Firestore document id in new backup output; a fresh `backup:database` run completed successfully on 2026-05-27; `restore:database` dry-run and apply both succeeded against the disposable Firebase project `my-journal-restore-drill`; the matching Firestore indexes plus rules were deployed successfully to that drill target; and the documented viewer recovery path was exercised successfully there (password reset, disable/reenable access, and restoration of the original password). The Firestore restore path plus viewer-account recovery are now credibly proven. **Storage byte backup script shipped 2026-06-12** (`npm run backup:storage`). **Deferred to post-review step 12 (last):** documented **Storage restore drill** on a disposable target, then Typesense/search parity on recovered targets, broader incident-response proof, and deployment/runtime validation on a recovered environment.
