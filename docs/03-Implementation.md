...
**Implemented slice — shell warmth and hierarchy** — The narrower semantic correction was selected. Journal Sidebar now uses the warm base background rather than the raised secondary background; Editorial remains unchanged. In Light this carries the paper tone through the shell while leaving white cards distinct. In Dark it lets the shell recede against the lighter brown closed cards without changing the successful opened reading surface. Twenty-four focused compiler tests, touched-code lint, and production typecheck pass. Live draft verification covered the Reader grid in both modes. Dark-to-Light mode switching loaded the correct black Light logo asset, so the earlier white-logo observation was not reproduced as a defect. The updated Journal preset is loaded as an unsaved draft for author review.
