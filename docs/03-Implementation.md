...
- **Closed utility-tile parity (2026-06-01)** - Closed no-cover utility cards should continue to read as one landscape-family feed group: `Question`, `Quote`, and `Callout` stay on the same `6:5` shell family and now share the same closed utility-surface family rather than letting `Callout` drift to a darker special-case background. Closed covered `Story` / `Gallery` title bands should also use that same question/utility surface across light and dark mode so the lower title panel reads as one consistent reader card family.
- **Closed utility-card headline / watermark alignment (2026-06-01)** - Within that shared utility-card family, closed `Quote` and `Callout` should visually relate to `Question`: quote body scale and callout title scale now sit on the same stronger utility headline step used by the closed question card; quote tiles use the same roomier centered headline band as question tiles, but keep their italic face; quote watermark tone stays in the same family as the question mark without matching its apparent darkness exactly; and closed callouts keep the centered watermark / vertical placement while returning their text stack to left alignment so they do not read like centered quote cards.
