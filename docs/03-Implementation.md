...
- **Authoring Flow Lab — Experimental branch prototype** — The reversible `codex/authoring-workflow-redesign` branch adds `/admin/studio-lab` as a read-only comparison surface while preserving `/admin/studio`. The Lab reads the existing Media and Tag providers, defaults its working-set request to Incomplete, and tests a continuous Review → Describe → Organize → Card → Complete sequence with stable Media selection and a persistent caption/tag/subject inspector. All inspector changes remain local; save, delete, tag mutation, Card creation, and Firestore writes are deliberately absent. A focused component test, touched-code lint, production typecheck, and live authenticated route/selection/tag-draft verification pass. This prototype is evidence for author review, not accepted replacement behavior.
