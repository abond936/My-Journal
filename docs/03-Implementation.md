...
- **Studio DnD architecture migration** - Establish one authored drag framework (`@dnd-kit`) with shared **Studio drag-domain** utilities/types, then migrate by domain instead of by pane: **Collections** first (tree/card bank/root/orphan flows), then **Compose** (children/gallery reorder), then **Media** bank → cover/gallery/body, then remove `react-dnd` from `CardForm`; keep `react-dropzone` only for file upload drop. Require stable overlays, stronger target states, and optimistic local commits to eliminate snap-back.
