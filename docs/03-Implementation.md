...
- **Access & privacy gate** - Re-verify hosted reader/admin boundaries in deployed use: direct URL behavior, hosted auth/session configuration, and absence of admin affordance leakage. Current highest-priority hosted finding: anonymous requests to `/view` and `/search` are still receiving page-route HTTP `200` while `/api/cards`, `/api/cards/search`, and `/api/view/media` return `401`, so the next implementation slice must diagnose and correct deployed route protection for those reader pages. The first attempted correction, explicit middleware matcher broadening for root and nested reader routes, has already been deployed and did not change the hosted result; the next approved code slice should investigate deployment/runtime bypass or page-route-specific behavior instead of repeating matcher-only edits. Local import helpers are expected to remain admin-only operational routes; audience-based reader sharing is future scope, not part of current v1 verification.
