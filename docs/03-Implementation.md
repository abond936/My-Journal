...
📐 **External codebase review handoff (2026-06-12)** - A full-repo review found strong integrity engineering, a coherent reader V2 + theme token foundation, and a credible Studio convergence direction. **Review program steps 1-12 closed 2026-07-10** (including **9a-9c**, partial **10/11**, and operational recovery). Residual **10/11** engineering slices remain incremental Phase 4 backlog — not a rewrite mandate.

📐 **Review program order** - **Complete 2026-06-12:** **(1)** engineering safety net, **(2)** reader performance, **(3)** backend hardening, **(4)** Studio legacy retirement, **(5)** reader mobile text edit (5a-5d), **(6)** E2E hardening, **(7)** API/service hardening, **(8)** pre-commercial security/ops. **Complete 2026-07-10:** **(12)** operational recovery closeout; **(9)** reader follow-ups (9a-9c); **(10)** Studio depth (partial); **(11)** engineering quality (partial strict-api gate). Residual **10/11** items remain incremental Phase 4 backlog.

📐 **Post-review program order (2026-06-12)** - Review program steps **9-12** closed 2026-07-10 (residual **10/11** engineering slices remain incremental). Continue one approved slice at a time on **demo-first** / Phase 4 `⭕1` items.
...
- **(8) Pre-commercial security and ops** - **Complete 2026-06-12:** **8a-8d** shipped (mutation rate limits, maintenance logging hygiene, viewer tag-count scoping, Sentry baseline).
- **(9) Reader follow-ups** - **9a shipped 2026-07-10:** dedicated `renditions.reader` (640px WebP) on import/replace, `getReaderDisplayUrl` prefers reader then studio then original, `npm run backfill:media-reader-renditions`. **9b shipped 2026-07-10 (assessment):** slice **2e-1** CSS containment retained; contract test on feed grid `data-feed-virtualization="css-containment"`; true DOM windowing deferred unless profiling shows DOM count is the bottleneck. **9c shipped 2026-07-10:** removed Explore sidebar `mounted` blank gate; stable shell + loading placeholder during tag hydration; AppShell sidebar defaults synced via `useLayoutEffect` + initial `matchMedia`.
- **(10) Studio depth** - **Shipped 2026-07-10 (partial):** typed `studioShellImperativeRegistry` + `registerBodyMediaInsert` on Studio shell context; `PhotoPickerFolderItem` extracted from `PhotoPicker`. **Remaining:** further `CardForm` decomposition (Story Assist panel) when a feature demands it.
- **(11) Engineering quality (incremental)** - **Shipped 2026-07-10 (partial):** `npm run typecheck:strict-api` (`tsconfig.strict-api.json` on `src/lib/api` + selected utils/registry). **Remaining:** per-directory TS strict rollout beyond API/utils, `cardService` facade split, unused-deps cleanup, broader `withApiRouteHandler` migration on legacy routes.
- **(12) Operational recovery closeout (last)** - **Complete 2026-07-10** on disposable **`my-journal-restore-drill`:** paired **`backup:database`** + **`backup:storage --apply`** runs (`run-2026-07-10T12-38-46-230Z` Firestore + `run-2026-07-10T12-41-08-324Z` Storage, 7078 objects); `restore:database` dry-run + apply (`--allow-non-empty` on prior drill data); Storage byte replay via Firebase Admin manifest upload (7078/7078); disposable Storage enabled + **`images/{allPaths=**}` read rules** so reader tiles load; local dev verification with `.env.test` (feed + card media); **`backup:run`**, guarded **`restore:storage`**, and **`restore:run`** shipped (**12d**). **12c shipped 2026-07-10:** `npm run build` + `npm run test:integrity -- --runInBand` pass on live workspace after drill closeout (193 tests). **Remaining before commercial release (not review-program blockers):** disposable-target Typesense rebuild with isolated config, hosted-runtime proof on recovered data, rollback/incident-response drills. Operator contract: `docs/NPM-SCRIPTS.md` -> **Restore drill**.
...
📐 **Review program step 5 — reader mobile text edit sequencing (2026-06-12)** - **Complete 2026-06-12** (5a–5d shipped):

- **5a Mobile quick-edit sheet** - Card detail title/subtitle/excerpt via `updateCardMetadata`. **Shipped 2026-06-12.**
- **5b Gallery caption edit** - Card detail gallery lightbox caption via `updateCardGallery` slot overrides. **Shipped 2026-06-12.**
...
- **8d Error monitoring** - Baseline production error visibility (Sentry). **Shipped 2026-06-12:** `@sentry/nextjs` + `sentryRuntime.ts`; production-only when DSN env vars set; errors-only (`tracesSampleRate: 0`); `beforeSend` scrubbing; `global-error.tsx`; `tunnelRoute: '/monitoring'` in `next.config.ts` (client events proxied through the app to reduce ad-blocker drops); optional source maps when `SENTRY_AUTH_TOKEN` + `SENTRY_ORG` + `SENTRY_PROJECT` set on the host build; regression in `sentryRuntime.test.ts`. **Operator:** set hosted env from `.env.example`; verify via Sentry project **Issues** (Send test event) after deploy — no example error route shipped.

📐 **Post-review step 12 — Operational recovery closeout sequencing (2026-06-12)** - **Complete 2026-07-10** on disposable **`my-journal-restore-drill`** (never production first):

- **12a Storage backup readiness** - **Shipped 2026-07-10:** `backup:storage --apply` run `run-2026-07-10T12-41-08-324Z` (`storage-manifest.json`, 7078 objects) paired with `backup:database` run `run-2026-07-10T12-38-46-230Z` (`firestore.json`). Database and Storage backups create **separate** `run-*` folders — pair by date, not folder name alone.
- **12b Disposable-target restore drill** - **Shipped 2026-07-10:** `DOTENV_CONFIG_PATH=.env.test` + `restore:database` dry-run then apply (`--confirm-project=my-journal-restore-drill`, `--allow-non-empty`); Storage bytes replayed from manifest via Firebase Admin upload (7078/7078). Enable Storage on the disposable project first; set Storage rules so **`images/**` is publicly readable** (repo `storage.rules` denies all client reads — live production already allows image reads; a fresh drill bucket needs explicit rules or images 403). **Not run:** `sync:typesense:fresh` / `sync:typesense:media:fresh` on the disposable target (production Typesense env must not be repointed accidentally).
- **12c Verification + canon closeout** - **Shipped 2026-07-10:** local dev with `.env.test`, admin sign-in, reader feed + card media on recovered data; **`npm run build`** + **`npm run test:integrity -- --runInBand`** pass (193 tests); stale `POST` audit entry for read-only `typesense-status` corrected in `apiRouteAccessAudit.ts`. **Remaining before commercial release (not review-program blockers):** disposable-target Typesense rebuild with isolated config; hosted-runtime proof on recovered data.
- **12d Restore helper** - **Shipped 2026-07-10:** guarded `restore:storage`, paired `backup:run`, and **`restore:run`** orchestrator.
...
📐 **Next reviewable slice handoff (updated 2026-07-11)** - **Studio/runtime browser verification closed 2026-07-11.** **Gallery reorder DnD closed 2026-07-11** (author verified on Studio shell Compose and reader content Compose modal). **Reader-modal children reorder closed 2026-07-11** (author verified): local `CardForm` `DndContext` commits `studioChild:*` reorder via `reorderChildrenIdsFromDragIds` + narrow `childrenIds` PATCH; `useChildCards` cache reorder avoids repaint on drop. **DnD hardening pass closed 2026-07-11** (author reports no further DnD issues in normal use). **Questions create/open → Compose handoff closed 2026-07-11** (author reports no flicker; reopen only if it resurfaces). Hosted Admin Responsiveness **typing/search/startup slices remain landed**; do not reopen that pass unless a new stall is reported. **Compose pane scroll containment closed 2026-07-11** (author verified). **Child rail feed-grid tiles closed 2026-07-11** (author verified). **No active demo-first blocker 2026-07-11** (author). **Next:** pick from Phase 3/4 `⭕1` polish when ready — not driven by a reported defect.
...
- **DnD hardening pass** - **Closed 2026-07-11 (author verified):** gallery reorder and reader-modal children reorder verified; author reports no further DnD issues in normal use. Reopen only on a new author-reported defect. Contract reference remains: structural tree drag with **on-row** nest/attach and **between-row** insert/reorder; `Media -> Cover` / `Media -> Gallery` as direct assignment; `Cards -> Children` and `Media -> Content` append-only cross-pane.
...
- **Closed-card matrix alignment (2026-05-31)** - The first Reader implementation slice is now in `V2ContentCard`: closed feed cards no longer render tag/context badges or content/gallery icon badges; only `Story` and `Gallery` retain a closed-card type badge; closed `Story` and `Gallery` supporting text is now subtitle-only by default instead of excerpt-first; and closed `Question` cards no longer render teaser/excerpt copy in the feed. **Updated 2026-07-10:** square feed tiles render dimensional tag chips in a fixed **bottom strip** on `Story`, `Gallery`, `Question`, and `Quote`; `Question`/`Quote` keep centered headline + watermark. **Shipped 2026-07-11:** bottom strip is **four fixed Who/What/When/Where slots** (`FeedTileChipStrip` + `buildFeedTileDimensionSlots`). Remaining matrix work: open-card header placement polish, `Question` closed-to-open continuity, unified Studio grid shell convergence.
...
- **Desktop feed rhythm normalization (2026-05-31)** - **Shipped 2026-07-10:** closed feed `Story`, `Gallery`, `Question`, and `Quote` tiles (non-inline) now resolve to one **1:1 square shell** at grid width via `usesSquareFeedTile` in `V2ContentCard`, replacing the prior mixed landscape/portrait height families for those types. `Story` remains cover-led with placeholder when no real cover exists. `Callout` still uses the utility portrait frame. **Cover-editor feed preview shipped 2026-07-10:** secondary 1:1 preview in `CoverPhotoContainer`.
...
- **Desktop feed rhythm verification checklist (2026-05-31)** - **Closed 2026-07-10** with square uniform feed tiles: main feed grid rows align at one square height for `Story`/`Gallery`/`Question`/`Quote`; meta bands align across types; `Callout` remains the portrait utility outlier. **Compact rail chip omission shipped 2026-07-10:** `size="small"` discovery/rails omit `FeedTileChipStrip`. **Cover-editor feed preview shipped 2026-07-10:** `CoverPhotoContainer` secondary 1:1 preview.
...
- **Cover framing contract** - Define one authoritative cover-framing target for authoring and reconcile Compose, reader feed, reader detail, and admin/Studio preview surfaces so focal adjustments do not look correct in one surface and wrong in another. Current agreed direction: Compose is the authoring truth for the primary cover presentation, open Reader should match that authored crop, and stored focal-point data drives alternate-shape secondary surfaces such as closed feed cards, rails, and `Explore More`. **Shipped 2026-07-10 (feed preview slice):** Compose `CoverPhotoContainer` includes a collapsible miniature `V2ContentCard` feed preview (default open) with live title/tags/cover beside framing controls. Open Reader detail follows the authored Compose crop; broader admin-grid parity review remains open.

📐 **Closed-tile presentation + tagging sequencing (agreed 2026-07-11)** - Canon in `docs/02-Application.md` (`📐 **Closed-tile dimensional chip contract**`, `📐 **Unified closed-tile shell direction**`, `📐 **Tag display vs aggregation architecture**`). Recommended implementation order:
  1. **Reader four-slot chip row** - `FeedTileChipStrip` / `V2ContentCard` main grid: always render Who/What/When/Where; empty = dimension-colored placeholder + `-`; filled = one label; no `+N`, no `zNA` on tiles.
  2. **Shared closed-tile shell extraction** - single module/variant for reader feed + Compose preview (same proportions; preview width follows feed column, not ad hoc scale).
  3. **Studio Cards grid convergence** - replace natural-aspect thumb + left tag rail with unified shell + admin overlay chrome (`AdminGridCellChrome` overlay slots only).
  4. **Rail size variants** - explicit compact tokens for Explore/rails; stop proportional shrink of medium tile. **Follow-up (deferred 2026-07-11):** **Utility typography surface parity** (below) — rail quote/question headline/watermark ratio still open.
  5. **Tag aggregation slice** - media-to-card suggest/apply pipeline, subject/LCA/span/mixed display rules, optional chip popover; separate from step 1.

- **Utility typography surface parity (deferred 2026-07-11; blocked on theme + surface contracts)** - Closed **Quote** / **Question** utility presentation is implemented in **four divergent paths** today: `/view` grid (`V2ContentCard`), discovery rails (`V2ContentCard` `size="small"`), Studio/Compose closed preview (`UtilityCardPreview` via `AdminClosedCardTileShell`), and open-card header/cover (`CardDetail.module.css`). Each uses different **`vw` / `rem` / `cqi` clamps** and headline tokens, so watermark and headline **ratio-to-tile** drifts across surfaces. **Do not patch rails ad hoc** until this slice defines one **utility headline + watermark token pair** (or shared module) consumed by all four contexts, with layout chrome only per surface. **Current rail status:** left as-is after 2026-07-11 attempts; known mismatch vs `/view` grid (headline too large, watermark too small / no grid-like overflow).

- **Closed-tile four-slot chip row (shipped 2026-07-11)** - `FeedTileChipStrip` / `buildFeedTileDimensionSlots` on reader main-grid tiles; see `docs/02-Application.md` -> `📐 **Closed-tile dimensional chip contract**`.
- **Unified closed-tile shell / Studio grid (shipped 2026-07-11)** - `AdminClosedCardTileShell` on Studio Cards grid; see `docs/02-Application.md` -> `📐 **Unified closed-tile shell direction**`.
- **Shared closed-tile shell extraction (Compose preview) (shipped 2026-07-11)** - `ComposeFeedTilePreview` uses `AdminClosedCardTileShell` + shared `closedCardTilePreviewUtils`; preview width follows main feed column (~300px); live slider crop passes through; tags from Compose form.
- **Rail size variants (shipped 2026-07-11; utility typography deferred)** - `--reader-feed-tile-rail-*` for compact badge/meta on rails; chip strip omitted. Quote/question rail headline/watermark parity **not closed** — see **Utility typography surface parity**.
- **Media-to-card tag aggregation (⭕1 Tag Management)** - Fifth slice; presentation must not block on it, but tile filled-state resolver should stay swappable when aggregation ships.
...
- **Tag count exposure on reads** - **Shipped 2026-06-12 (8c):** `tagApiProjection.ts`; viewer tag GET routes omit operational count/id-set fields; regression in `tagApiProjection.test.ts`.
- **Error monitoring** - **Shipped 2026-06-12 (8d):** opt-in Sentry baseline; production-only when DSN env vars set; errors-only; secret scrubbing; `tunnelRoute: '/monitoring'`; optional source-map upload when build auth env set. Hosted operator setup: `.env.example` + Sentry **Issues** test event after deploy.
...
- **Operational recovery gate** - Verify database backup, local secrets backup, restore drill, rollback/incident response, and admin account recovery before commercial release. Current closeout: **Firestore + Storage byte restore proven 2026-07-10** on disposable `my-journal-restore-drill` (paired backup runs, `restore:database` apply, Admin manifest Storage replay 7078/7078, reader media verified locally with `.env.test`; **`backup:run`**, **`restore:storage`**, **`restore:run`** shipped; post-drill **`npm run build`** + **`npm run test:integrity -- --runInBand`** pass). **Remaining before commercial release:** rollback/incident-response drills, disposable-target Typesense rebuild with isolated Typesense config, hosted-runtime proof on recovered data. Operator path: `docs/NPM-SCRIPTS.md` -> **Restore drill**.
