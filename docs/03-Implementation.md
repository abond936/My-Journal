...
- **Family demo milestone** - Near-term priority is a credible family demo of the **private hosted journal**: smooth reader flow, dependable author prep, and no obvious integrity or trust failures.
- **Interim development step** - Treat the family demo as an explicit interim milestone used to validate product direction, interaction quality, and trustworthiness before broader feature expansion or commercialization.
📐 **Demo-first execution stance (canonical)** - Until the family demo is credible, prioritize work in this order: **reader stability/polish**, **hosted access/privacy correctness**, **author prep workflow friction**, **import coherence**, then broader platform or aesthetic expansion. Do not let Theme Management breadth or speculative product extensions outrank the demo path unless they directly block it.

ðŸ“ **Demo-first execution stance (2026-05-22)** - Until the family demo is credible, prioritize work in this order: **reader stability/polish**, **hosted access/privacy correctness**, **author prep workflow friction**, **import coherence**, then broader platform or aesthetic expansion. Do not let Theme Management breadth or speculative product extensions outrank the demo path unless they directly block it.
...
ðŸ“ **Admin DnD convergence plan (2026-05-22)** - Treat admin DnD convergence as platform work, not isolated bug fixing. Build against `docs/02-Application.md` → `ðŸ“ **Admin DnD architecture contract (2026-05-22)**`. Execution order: **Assignment** first (Studio media/card relationship placement), then **Editor-integrated** (body insertion and in-body moves as a separate subsystem), then **Local reorder**, then remaining **Taxonomy/Structural** polish. Do not expand admin DnD breadth while any surface still depends on hybrid ownership between shell, pane, and editor logic.
...
📐 **Canonical task intake (2026-05-23)** - Do not let chat become a shadow backlog. Use chat for intake, audits, clarifications, and approvals; keep canonical product/status truth in `docs/02-Application.md` and canonical active sequencing truth in `docs/03-Implementation.md`. When ad hoc notes reveal real remaining work, normalize them into the owning `02` section and, if they are true next-step work, mirror them here as `⭕1` items rather than leaving them in prompts or chat history.
...
📐 **Studio status addendum (2026-05-05)** - Recent closeout tightened operator correctness and reduced browser-native seams without changing the underlying Studio domain split: Questions now act as the practical question-admin surface (quick tags, Save/Cancel, in-pane delete/open/unlink, collapsible list, wider pane), Compose delete is live and now reconciles local card-bank state, Studio destructive flows on migrated paths use app confirmation dialogs plus panel-anchored activity overlays, collection-tree expansion now keys off structural `childrenIds`, and the Studio Cards pane no longer force-pins a selected card to the top when it no longer matches the active filters/status.
📐 **Shell + feedback addendum (2026-05-21)** - Wave 1 shell cleanup is now in place across the shared app chrome: login, reader header, and landing branding use the revised `Title-light2` / `Title-dark2` artwork; visible admin tabs are removed in favor of the shared hamburger; header and sidebar chrome are tightened to reclaim vertical working room; and the app now has one shared feedback layer for migrated surfaces (`success` transient toast, `error` persistent dismissible toast, themed confirm/alert dialogs, local loading overlays). Remaining pane-specific browser-native prompts should be treated as follow-on Studio/admin migration work, not as the intended contract.
📐 **Studio filter framing (2026-05-07)** - Current wording should read as **pane-local workspace filters** for Cards and Media, not as a global Organize-rail filter over the whole Studio shell; the Cards and Media panes each define their own working set inside Studio's multi-domain layout.
📐 **Studio width containment (2026-05-07)** - The right-side **Compose / Questions / Media** workspace now decides between resizable desktop row and stacked fallback from its own remaining width after **Organize** and **Cards**, so panes and media tiles stay inside the shell instead of overrunning the right edge when the app is still broadly desktop-wide.
📐 **Studio shell width containment (2026-05-08)** - The higher-level embedded `CollectionsAdminClient` shell now uses its own actual available width, not just the viewport breakpoint, when deciding whether `Organize | Cards | workspace` can remain in the desktop multi-column layout. This is the authoritative fix for the remaining right-edge overflow because it corrects the first container that could force impossible widths downstream.

📐 **Studio ergonomics addendum (2026-05-21)** - The next Studio pass is now applied on the operator surfaces themselves: the right-side workspace holds its side-by-side layout longer before wrapping, pane toolbars and search rows are denser, Cards and Media expose their titles inline with the working controls, redundant embedded pane labels are removed where the top chrome already identifies the surface, and Questions plus Users now consume the shared feedback layer instead of pane-local success/error banners. This pass intentionally does **not** change pagination vs infinite-scroll behavior or the deeper Cards↔Media relationship model.
📐 **Studio relationship-edit addendum (2026-05-22)** - Guided leaf selection now opens the leaf card directly instead of dropping the reader into an empty collection feed, and Studio's next relationship workflow step is now live as a focused media editor modal: selected-card media can be opened from Compose into asset-level editing, media references can jump directly into card Compose, and the intended caption contract is now `media caption by default, card override only when intentionally exceptional`.
📐 **AI Story Assist addendum (2026-05-22)** - The current AI writing surface is now explicitly framed as `AI Story Assist` rather than generic assist. The first named-guide pass is live in `CardForm`: `Bob` and `Sandra` are selectable guides with a persisted local preference and a light prompt-level tone difference only; capability and factual constraints remain shared across both guides. The next expansion pass adds descriptive current-card modes in the same surface: `Draft from notes`, `Tighten wording`, `Expand this memory`, `Retitle this story`, and the first coaching mode, `Make this story stronger`, which returns structured story-improvement suggestions rather than a replacement draft.
📐 **Studio consistency addendum (2026-05-22)** - The latest operator pass standardizes the compact pane contract rather than adding new domains: Cards, Media, and Questions now share hint-first filter fields, blue-on-white `Edit` / `Clear` actions, tighter common control typography, and a more explicit tag-filter order (`Edit tags...` first, selected dimensional tags visible, rules immediately below). Cards and Media no longer hide their one-line rules row behind a second toggle, full tag editing is reduced to one selected-state presentation plus a four-column tree sized for the current taxonomy, and path labels now de-duplicate repeated stored ancestors so suggestion strings stay readable.
📐 **Admin media browsing addendum (2026-05-22)** - Media admin now matches the preferred operator browsing posture more closely: the visible working set grows through append-style loading with auto-load near the bottom plus a `Load more` fallback, while the underlying provider still honors the existing cursor / seek / Typesense constraints and keeps local update/delete reconciliation on the loaded set. Cards were already on the append-style path and did not require a model change.
📐 **Studio shell polish addendum (2026-05-22)** - A smaller shell follow-up removed the duplicate embedded `Cards` heading, restored right-side pane resize handles after refresh whenever multiple panes are visible, and kept unlink/type-conversion changes from leaving the active Compose form in stale `Question` state.
...
ðŸ“ **Backlog capture addendum (2026-05-23)** - The current docs must carry the full UI/workflow backlog rather than leaving active product truth in scratchpads or chat. The newly captured items now span: shared admin button standardization; header logo scale and hamburger spacing; sticky reader dimension shortcuts; Studio pane sizing presets and independent pane resizing; Organize dimension shortcuts, tree-density follow-up, and collection count placement; Card/Compose spacing, cross-linking, structure counts, delete reconciliation, action placement, AI collapse behavior, excerpt/live-writing support, drop cap, read/edit convergence, hero framing, and assigned-media visibility; Questions add-button prominence, default tree collapse, assigned terminology, inline editing, open-on-select, and include-children counting; Media infinite-scroll hardening, card/media cross-linking, cards/media workspace parity, and caption clamp; User Management title/casing/alignment cleanup; and the open `Subject` tag-model decision in `docs/02-Application.md`.
