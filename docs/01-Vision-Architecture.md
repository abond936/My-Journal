...
- **Quality** - QA app.
- **Security Hardening** - Threat-model review, authorization review, secret-handling review, and hosted deployment hardening for commercial readiness. Current boundary: `storage.rules` is referenced by `firebase.json` but not yet present in the repo; add `.env.example` for onboarding/deploy documentation; retire the env-password auth fallback at rollout.
...
- **Operational recovery gate** - Verify database backup, local secrets backup, restore drill, rollback/incident response, and admin account recovery before commercial release. Current boundary: the core Firestore restore path and viewer recovery flow have been exercised successfully on a disposable target; remaining work includes Firebase Storage byte backup/recovery (see Backup **Storage byte backup**), Typesense/search parity on recovered targets, broader incident-response proof, and deployment/runtime validation on a recovered environment.
