...
- **Operational recovery gate** - Verify database backup, local secrets backup, restore drill, rollback/incident response, and admin account recovery before commercial release. Current boundary: Firestore restore + viewer recovery proven on disposable target; **Storage byte backup script shipped 2026-06-12** (`npm run backup:storage`). **Next:** post-review **step 12** — full disposable-target Storage restore drill + Typesense rebuild + verification (`03` step **12a-12c**; operator path in `docs/NPM-SCRIPTS.md`).
