...
- **Operational recovery gate** - Verify database backup, local secrets backup, restore drill, rollback/incident response, and admin account recovery before commercial release. Current boundary: the core Firestore restore path and viewer recovery flow have been exercised successfully on a disposable target; **Storage byte backup script shipped 2026-06-12** (`npm run backup:storage`); remaining work includes a documented Storage restore drill, Typesense/search parity on recovered targets, broader incident-response proof, and deployment/runtime validation on a recovered environment.
