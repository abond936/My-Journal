...
- **Card Content Presentation**
  ...
  - Closed reader feed tiles (`Story`, `Gallery`, `Question`, `Quote` in non-inline modes) use a **uniform 1:1 square shell**: cover or type-native upper band, a title band where applicable, and a fixed **bottom tag-chip strip** (dimensional chips via `buildReaderCardPresentation`; spacer when no tags resolve). `Question` and `Quote` keep title + watermark **optically centered on the full tile** (chip strip excluded from center math). Type overlay badges remain on closed `Story` and `Gallery` only; excerpts stay excluded from reader card presentation for now.
...
- **Square uniform feed tiles (shipped 2026-07-10)** - Closed feed `Story`, `Gallery`, `Question`, and `Quote` tiles (non-inline) share one **1:1 square shell** in `V2ContentCard` via `usesSquareFeedTile`: image or type-native upper band, title band (`FeedTileMetaBand`) where applicable, and a fixed bottom **tag-chip strip** (`FeedTileChipStrip`; spacer when empty). `Question` and `Quote` keep headline + watermark optically centered on the full tile. `Callout` keeps the prior utility portrait shell. Cover-editor feed-preview parity and user-facing tile prefs remain future work.
