...
📐 **Studio orchestration decisions (2026-04-20)** - (1) **Tags:** North-star layout is an expanded admin left rail with full Tag Admin capabilities (reorder, collapse/expand, add/delete/edit) on the same canonical `TagProvider` tree—no second taxonomy model; `/admin/tag-admin` stays until the rail fully replaces those workflows. (2) **Card edit:** Do not embed full `CardForm` (or the full card edit page) inside Studio as the immediate path; keep deep authoring on `/admin/card-admin/[id]/edit` (deep-link or split view) until column widths and workspace shell stability are proven; Studio focuses on relationship orchestration and inspection. (3) **Media:** The right column uses **one** media surface: the same `MediaProvider`-driven `CollectionsMediaPanel` (compact) as full Collections, with rows registering as `source:*` drags into Cover/Gallery for the selected card (**Studio media bank parity**, 2026-04-21); the prior ad hoc `GET /api/media?limit=40` strip is removed.
...
- **Studio** - Admin route `/admin/studio`: experimental combined shell — left embeds the same tag DnD tree as Tag admin (`useTagManagement` + `TagAdminList`); center is `CollectionsAdminClient` (`embedded`) in a **three-column** layout (curated tree · unparented · **selected card context + Media bank**) inside one `DndContext` with Collections: cover/gallery/source media DnD, **child-card reorder** (sortable `studioChild:*` ids), **attach** by dropping a tree/unparented card onto the Children block (`parent:{selectedCardId}`), inline tags, and the same **`CollectionsMediaPanel`** as Collections (compact list, filters, pagination, bulk delete) with bank rows draggable to Cover/Gallery. Successful card `PATCH` refreshes the embedded collections list. Collections and Tag admin routes unchanged; see `📐 **Studio orchestration decisions (2026-04-20)**`.
