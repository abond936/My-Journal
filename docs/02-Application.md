...
- **Reader mobile text edit** - Allow admin users on `/view` to make narrow text edits (title, body, caption) through lightweight reader-path UI and narrow PATCH routes, without requiring full Studio or the current desktop-oriented edit modal for routine touch-ups. **Shipped 2026-06-12 (slice 5a):** card detail (`/view/[id]`) opens a mobile quick-edit sheet (≤768px) for title/subtitle/excerpt via `updateCardMetadata`; desktop keeps full `ReaderCardEditModal`. **Shipped 2026-06-12 (slice 5b):** card detail gallery lightbox caption edit for admins via `updateCardGallery` slot overrides (Studio Media default preserved when unchanged). Body and feed-tile quick edit remain follow-up (5c–5d).
