...
- **Studio card bank dimensional filters** - Attach-candidate bank filters cards by title/status/sort and by **on-card** vs **on-attached-media** dimensional tags (`who`… vs `mediaWho`…) using merged catalog + `listCuratedTreeAttachCandidates`.
- **Studio embedded media dimensional control** - Media column: filters apply to **media documents**; optional merge of compose card dimensional tags into **`GET /api/media`** is **off by default** in Studio (`studioMediaMergeCardDimensionalTags`).
...
- **Admin card grid layout** - Card admin **grid** (including Studio attach bank): dimensional tags in a **left vertical rail** (Who→What→When→Where; no repeated column labels—order explained via tooltip); removable tags + **narrow left-aligned** tag search row (`CardDimensionalTagCommandBar` **search-only** variant); **natural-aspect** cover thumbnails (`width`/`height` → `aspect-ratio`, `object-fit: contain`); header row for checkbox, type/status, delete, and Studio drag handle when present; **excerpt/subtitle** as caption under the image where applicable (Studio compact grid hides duplicate title row—full title on cell tooltip).
...
📐 **Studio media & body (2026-04-22)** - **Unparented / attach candidates:** the **Cards** column **Attach candidates** bank (filters + sort + **on-card** vs **media-on-card** dimensional filters) fulfills the candidate list role—no separate **unparented** column required. **TipTap body images:** keep **clipboard paste**; add **Media admin → body** when **`TipTap body media from bank (Studio)`** lands. **Cover / gallery / children in Studio:** **Media admin** is the library surface—**drag** to targets on **in-shell Card Edit** (**`Studio selected-context elimination (v1)`**). **PhotoPicker:** **retain** in full-page card edit until **`PhotoPicker convergence in Media admin`** supplies bank import + library pick parity; **end state** is **no parallel picker** for Studio and eventually card edit.
...
- **Grid tag editing UX** - Media grid cards provide `Edit tags…` opening the existing searchable tag selector (select/remove + save) without switching to table mode; **dimensional tags** on the tile use the same **vertical rail** pattern as card grid (read-only display; order tooltip).
- **Admin media grid layout** - **Natural-aspect** thumbnails (`width`/`height` → `aspect-ratio`, `object-fit: contain`); **caption** under the image; **checkbox + Studio drag handle** on a top header row (Studio); layout mirrors card grid **rail | image** structure.
