...
- **Hosted route-protection mismatch (observed)** - On the current hosted deployment, anonymous requests to `/view` and `/search` still reach the page routes with HTTP `200`, while the corresponding reader APIs reject anonymous access with `401` and `/admin` still redirects to login. The explicit middleware matcher broadening for root and nested reader routes has already been deployed and did not change that hosted result. Treat this as an active production mismatch, not as completed reader-boundary behavior.
...
- **Reader-only auth path** - Intended contract: reader routes (`/view`, `/view/*`, `/search`) require an authenticated session; reader APIs require a session; viewers can read only published cards and published child cards, while admins retain draft visibility. Protected reader routes keep their shell chrome during client session hydration so server/edge auth truth and visible reader navigation do not drift apart. Current hosted mismatch: page-route protection for `/view` and `/search` is not yet fully aligned with that intended contract, even though the reader APIs are rejecting anonymous access. The first direct middleware matcher broadening has already been attempted and deployed without resolving the hosted reader-page `200` responses, so the remaining issue should be treated as a deeper route-protection/runtime diagnosis rather than an already-proven matcher typo. Admin-only local import helpers and other admin operational APIs stay outside the reader surface.
...
- **Card/media cross-linking** - Make card and media working surfaces jump directly to each other from relationship references so assigning, checking, and correcting links is faster.
- **Relationship counts in cards** - Expose compact parent/child counts on card rows/tiles (for example `Prnt/Chld (x/y)`) so structure is visible without opening the card.
- **Compose deletion reconciliation** - When a card is deleted from Cards, clear or replace the active Compose context immediately instead of leaving stale card content visible.
- **Compose action placement** - Move Compose primary actions into a stable upper-right action area.
- **AI Story Assist collapse state** - Keep `AI Story Assist` collapsed by default when not active so it does not consume Compose working space.
...
- **Card/media cross-linking** - Make media and card working surfaces jump directly to each other from relationship references so assigning, checking, and correcting links is faster.
...
- **Assigned terminology parity** - Rename Questions `Included / Not included` to `Assigned / Unassigned` to match media language.
- **Inline question editing** - Let the question prompt itself edit inline without a separate edit button.
- **Question open-on-select** - Make question selection open the item immediately, matching cards/media behavior.
- **Question include-children toggle** - Add an `Include children` toggle and keep displayed counts consistent with the active tree scope.
