...
- **Hosted role verification** - Hosted session checks now confirm the primary route and API boundary contract across signed-out, `viewer`, and `admin` use: `viewer` sessions can access reader routes and reader APIs, do not see admin navigation affordances on reader surfaces, and are redirected away from representative admin routes while admin-only APIs return `403`; `admin` sessions can access both reader and admin surfaces and the corresponding admin API. No concrete hosted access/privacy leak was found in this pass.
...
- **Navigation** - The shared hamburger keeps reader destinations for all authenticated users, while admin-only entries (`Studio`, `Users`, `Theme`) appear only for admin sessions; viewers do not see admin navigation affordances on reader surfaces.
