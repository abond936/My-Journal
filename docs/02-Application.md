...
- **Hosted route-protection mismatch (observed)** - On the current hosted deployment, anonymous requests to `/view` and `/search` still reach the page routes with HTTP `200`, while the corresponding reader APIs reject anonymous access with `401` and `/admin` still redirects to login. Treat this as an active production mismatch, not as completed reader-boundary behavior.
