...
- **Grid tag editing UX** - Media grid cards provide `Edit tags…` opening the existing searchable tag selector (select/remove + save) without switching to table mode; **dimensional tags** on the tile use the same **`AdminGridCellChrome`** left rail as card grid (**read-only**; one clipped name per dimension + **`+`** when multiple; **native `title`** on rail, each dimension row, **cell**, and **thumbnail** lists all names per line).
