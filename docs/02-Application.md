...
- **Operator Tooling** - Scripts, backup, and recovery helpers remain part of the administration lifecycle while first-class admin UX continues to mature. **Operational recovery closeout (review step 12) shipped 2026-07-10:** Firestore + Storage byte restore proven on disposable `my-journal-restore-drill`; **`backup:run`**, **`restore:storage`**, **`restore:run`** shipped; **12c** build + integrity verification pass. **Review program steps 9-11 remain active next work** (see `03`). Canonical operational detail lives in `01-Vision-Architecture.md`, `docs/03-Implementation.md` step **12**, and `docs/NPM-SCRIPTS.md`.
