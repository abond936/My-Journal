...
- **Domains** - Cards, Media, Tags, Questions, Users, Themes, and **Collections** (curated tree) are active. **Studio** is shipped as an **embedded shell inside Collections** (`/admin/collections` Studio mode): Organization (tags/tree), **Cards** (attach-candidate bank + filters), **Media** (full embedded `MediaAdminContent`), **Compose** (in-shell `CardForm` + cover/gallery/children DnD from the media bank). Remaining **⭕1** work: IA demotion, full **Studio tag rail**, and inline-tag/modal parity called out below. Canonical contracts: `📐 **Studio unified shell contract**`, `📐 **Studio media & body (2026-04-22)**`.
...
- **Studio desktop shell (v1)** - Embedded Collections Studio: multi-pane layout (Organization · Cards · Media · Compose) and one session-scoped selection model (`cardId`, media multiselect) per `📐 **Studio unified shell contract**`.
...
- **Studio card edit** - Collections Studio **Compose** column embeds **`CardForm`** + **`CardFormProvider`** for the session-selected card (`StudioShellContext`); **PhotoPicker** + TipTap **clipboard paste** unchanged; delete/duplicate/full chrome via linked full-page Card admin (`StudioCardEditPane`). **DnD:** cover, gallery, and children accept drops from the embedded media bank on the same **`CollectionsAdminClient`** `DndContext` (**`Studio selected-context elimination (v1)`** in **Administration**). **PhotoPicker** superseded over time by **Media admin** + **`PhotoPicker convergence in Media admin`**.
...
📐 **IA vs Studio** - Full-page `/admin/collections` remains the structural tree surface today; once Studio’s curated pane is the default workflow, **primary admin navigation** should not advertise this route separately (code may remain until removal is safe).
...
- **Triage** - Admin route `/admin/media-triage`: three-pane workspace (queue · preview/caption · media tags) over the same `MediaProvider` / `GET /api/media` list as Media admin; **target:** superseded by Studio media workflows—demote from primary admin IA per **Studio IA demotion** and `📐 **Studio unified shell contract**` (6).
- **Studio** - **Primary entry:** Collections Studio mode (`CollectionsAdminClient` `embedded`): tag admin tree (`useTagManagement` + `TagAdminList`), curated tree + attach-candidate **Cards** bank + embedded **`MediaAdminContent`** (`studioSourceDraggable`) + in-shell **`CardForm`** **Compose** column, and Collections-style DnD (cover/gallery/children). Standalone route `/admin/studio` may remain as a redirect or legacy entry—product truth is the **embedded** shell (`📐 **Studio unified shell contract**`).
